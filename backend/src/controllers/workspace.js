...
const getWorkspacesStats = async (req, res) => {
  try {
    const { workspaceId } = req.params;
...
const inviteUserToWorkspace = async (req, res) => {
  try {
    ...
    const notification = new Notification({
      user: existingUser._id,
      type: "workspace",
      content: `You have been invited to join workspace "${workspace.name}" by ${req.user.name}`,
    ...
    if (io) {
      console.log(`📢 Emitting workspace invite notification to user:${existingUser._id}`);
      io.to(`user:${existingUser._id}`).emit("new_notification", {
...
const acceptGenerateInvite = async (req, res) => {
  try {
    ...
    if (isMember) {
      return res
        .status(400)
        .json({ message: "User already member of this workspace!" });
    }
